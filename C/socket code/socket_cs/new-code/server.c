...
void send_data()
{
  ...
  signal(SIGPIPE,SIG_IGN);
    ...
    if (cl_head == NULL) {
      ...
      temp = cl_head;
      printf("Enter type of data to send (0: Hi message) :\n");
      scanf("%d",&data_type);
      while (temp != NULL) {
        /* Write a response to the client */
        bzero(sendBuff,sizeof(sendBuff));
        ticks = time(NULL);
        snprintf(sendBuff,sizeof(sendBuff), "%.24s#%d#%d#Server says Hi..[Sequence_num: %d]",ctime(&ticks),
                 data_type, 0, data_seq_num++);
...
*/
  while(1) {
    ...
    printf("\nEnter your choice:: \n 0. Exit \n 1. Send data to client \n"
      " 3. Print client list\n 4. Print mcast group list \n 5. Print mcast_group wise client list\n");
    scanf("%d", &choice);
